...
| 文件 | 行数 | 职责 |
|---|---|---|
| `tools/tts.py` | 171 | 主程序：加载模型 → 按句切分 → 逐句合成 → ffmpeg 拼接 → 输出 wav/mp3 |
| `tools/check.py` | 83 | 环境自检：Python 版本、依赖、模型文件、显存/内存 |
| `install.bat` / `一键安装.bat` | 102 / 101 | 建 venv、装依赖、下模型（首次 8–12G，看下载源） |
| `start.bat` / `一键生成.bat` | 100 / 95 | 找 Python → 调 `tts.py` → 报错时给中文提示 |
| `bench-long/bench_long.py` | 150 | 长文本实测脚本（走同一条 tts.py 路径，采样耗时/内存） |
...
| 磁盘 6.94G（模型 5.05G + 环境 1.88G） | `logs/runtime-memory-disk.txt` | 目录递归实测 |
...
| 15 / 30 分钟两组新数据 | `bench-long/result.txt` | 长文本实测原始输出 |
